...
namespace CSObjectWrapEditor
{
    ...
    public static class GeneratorConfig
    {
        ...
        #else
            //public static string common_path = Application.dataPath + "/common/XLua/Gen/";
            public static string common_path = Application.dataPath + "/Scripts/Lua/Gen/";
        #endif
    ...
    public static class Generator
    {
        ...
        //黑名单
        public static List<KeyValuePair<string, List<string>>> BlackList = null;
        //白名单
        ...
        static void getClassInfo(Type type, LuaTable parameters)
        {
            ...
            if (!type.IsAbstract)
            {
                foreach (var con in type.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.IgnoreCase).Cast<MethodBase>()
                         .Where(constructor => GeneratorUtils.IsOutExport(constructor)))
                {
                    int def_count = 0;
            ...
            parameters.Set("methods", type.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.IgnoreCase | BindingFlags.DeclaredOnly)
                ...
                .Where(method => !IsDoNotGen(type, method.Name))
                           .Where(method => GeneratorUtils.IsOutExport(method) && (!method.IsGenericMethod || extension_methods.Contains(method) || isSupportedGenericMethod(method)) && !method.Name.StartsWith("op_") && !method.Name.StartsWith("add_") && !method.Name.StartsWith("remove_"))
            ...
            parameters.Set("getters", type.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.IgnoreCase | BindingFlags.DeclaredOnly)
                           ...
                           .Where(prop => prop.CanRead && (prop.GetGetMethod() != null)  && prop.Name != "Item" && GeneratorUtils.IsOutExport(prop)).Select(prop => new { prop.Name, IsStatic = prop.GetGetMethod().IsStatic, ReadOnly = false, Type = prop.PropertyType })
                .Concat(
                    type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.IgnoreCase | BindingFlags.DeclaredOnly)
                               .Where(field => GeneratorUtils.IsOutExport(field))
            ...
            parameters.Set("setters", type.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.IgnoreCase | BindingFlags.DeclaredOnly)
                           .Where(prop => prop.CanWrite && (prop.GetSetMethod() != null) && prop.Name != "Item" && GeneratorUtils.IsOutExport(prop)).Select(prop => new { prop.Name, IsStatic = prop.GetSetMethod().IsStatic, Type = prop.PropertyType, IsProperty = true })
                .Concat(
                    type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.IgnoreCase | BindingFlags.DeclaredOnly)
                               .Where(field => GeneratorUtils.IsOutExport(field) && !field.IsInitOnly && !field.IsLiteral)
            ...
            parameters.Set("events", type.GetEvents(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.IgnoreCase | BindingFlags.DeclaredOnly).Where(e => GeneratorUtils.IsOutExport(e))
        ...
        static void GenOne(Type type, OnCallBackType_LuaTable type_info_getter, XLuaTemplate templateAsset, StreamWriter textWriter)
        {
            if (GeneratorUtils.isObsolete(type)) return;
                ...
                if (wrap_type.IsEnum)
                {
                    GenOne(wrap_type, (type, type_info) =>
                    {
                        ...
                        type_info.Set("fields", type.GetFields(BindingFlags.GetField | BindingFlags.Public | BindingFlags.Static)
                                      .Where(field => !GeneratorUtils.isObsolete(field))
                            .ToList());
                    }, templateRef.LuaEnumWrap, textWriter);
                }
        ...
        public static void GenLuaRegister()
        {
            GenLuaRegister(false);
        }
        public static void GenLuaRegister(bool minimum)
        {
        ...
        public static void AllSubStruct(Type type, OnCallBackType cb)
        {
        ...
        static void MergeCfg(MemberInfo test, Type cfg_type, Func<object> get_cfg)
        {
            ...
            if (test.IsDefined(typeof(HotfixAttribute), false))
            {
                ...
                if (cfg is IEnumerable<Type>)
                {
                    ...
                    {
                        if (!HotfixCfg.ContainsKey(type) && !GeneratorUtils.isObsolete(type) 
            ...
            {
                BlackList.AddRange(get_cfg() as List<KeyValuePair<string, List<string>>>);
            }
        ...
        public static void GetGenConfig(IEnumerable<Type> check_types)
        {
            ...
            BlackList = new List<KeyValuePair<string, List<string>>>()
            {
            };
            ...
            LuaCallCSharp = LuaCallCSharp.Distinct()
                                         .Where(type=>/*type.IsPublic && */!GeneratorUtils.isObsolete(type) && !type.IsGenericTypeDefinition)
        ...
        public static void GenCodeForClass()
        {
            GenCodeForClass(false);
        }
        public static void GenCodeForClass(bool minimum)
        {
