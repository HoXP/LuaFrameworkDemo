...
namespace XLua
{
    ...
    public class LuaEnv : IDisposable
    {
        ...
        private static List<OnCallBackLuaEnv_ObjectTranslator> initers = null;

        public static void AddIniter(OnCallBackLuaEnv_ObjectTranslator initer)
        {
            if (initers == null)
            {
                initers = new List<OnCallBackLuaEnv_ObjectTranslator>();
            }
            initers.Add(initer);
        }
        ...
        public T LoadString<T>(byte[] chunk, string chunkName)
        {
            return LoadString<T>(chunk, chunkName, null);
        }
        public T LoadString<T>(byte[] chunk, string chunkName , LuaTable env)
        {
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnvLock)
            {
        ...
        public T LoadString<T>(string chunk)
        {
            return LoadString<T>(chunk, "chunk");
        }
        public T LoadString<T>(string chunk, string chunkName )
        {
            byte[] bytes = System.Text.Encoding.UTF8.GetBytes(chunk);
            return LoadString<T>(bytes, chunkName, null);
        }
